...
//Toast End
  ...
  const signinUser = () => {
    if (role == '' || role.length == 0) {
      Toast.fire({icon:'warning',title:'Select Role'})
    }
      ...
      Toast.fire({icon:'warning',title:'Please Enter Password'})
      ...
      Toast.fire({icon:'error',title:'Error while Logging In'})
        ...
        axios.post(`http://localhost:8080/vendor/login`, data).then((response) => {
          ...
          if (result) {
            ...
            Swal.fire({icon:'success',title:'Successfully Logged IN '})
