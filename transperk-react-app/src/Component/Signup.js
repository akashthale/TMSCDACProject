...
const Signup = () => {
  ...
  const signup = (() => {
    ...
    if (role == "" || role.length == 0) {
      Toast.fire({icon:'warning',title:'Please enter Role'})
    }
    else if (name.length == 0) {
      setFnameErr(true)
      setFnameErrMsg("please enter name")
      Toast.fire({icon:'warning' , title: "Plase Enter first name"})
    }

    else if (email.length === 0) {
      setEmailErr(true)
      setEmailErrMsg("please enter email")
      Toast.fire({icon:'warning' ,title:'Please Enter Email'})
    } else if (!validEmail.test(email)) {
      setEmailErr(true)
      setEmailErrMsg("please enter valid email")
      Toast.fire({icon:'warning' ,title:'Please Enter Valid Email'})
    }
      ...
      Toast.fire({icon:'warning' ,title:'Please Enter phone number'})
      ...
      Toast.fire({icon:'warning' ,title:'Please Enter Valid phone number'})
      ...
      Toast.fire({icon:'warning' ,title:'Please Enter Address'})

    } else if (city.length === 0) {
      Toast.fire({icon:'warning' ,title:'Please Enter City'})
      // alert("please enter city")
    } else if (state.length === 0) {
      // alert("please enter state")
      Toast.fire({icon:'warning' ,title:'Please Enter State'})

    } else if (postalCode.length === 0) {
      // alert("please enter postal code")
      Toast.fire({icon:'warning' ,title:'Please Enter Postal Code'})

    } else if (postalCode.length > 6) {
      // alert("Postal Code Cannot be greater than 6")/
      Toast.fire({icon:'warning' ,title:'Postal Code Cannot be Greater than 6'})

    } else if (postalCode.length < 6) {
      // alert("Postal Code Cannot be less than 6")
      Toast.fire({icon:'warning' ,title:'Postal Code Cannot be Less than 6'})
      ...
      if (role == "CUSTOMER") {
        ...
        axios.post(`http://localhost:8080/customer`, body).then(response => {
          ...
          if (result) {
            Swal.fire({icon:'success',title:'Congratulations',footer:'User Created !'})
            history.push('/Signin')
          }
          else {
        ...
        axios.post(`http://localhost:8080/vendor`, body).then(response => {
          const result = response.data;
          if (result) {
            Swal.fire({icon:'success',title:'Congratulations',footer:'Successfully Logged in'})
            // alert('succcess')
            history.push('/Signin')
          }
          else {
